...
import { PREVIEW_COLORS } from './estimatorPreviews';
...
export default function CostEstimator() {
  ...
  const summary = `Backsplash project: ${areaLabel} sq ft; ${pattern}; ${material}; color: ${colorLabel}; existing backsplash removal: ${demolition ? 'yes' : 'no'}. Measurement method: ${method === 'cabinets' ? `${cabinets} cabinets at ${cabinetWidth} inches wide, ${height} inches high` : method === 'area' ? 'known area' : `${length} feet long, ${height} inches high`}. Installation: ${estimate?.installation != null ? money(estimate.installation) : 'personal quote required'}. Demolition and new drywall: ${money(estimate?.preparation ?? 0)}. Labor price for selected scope: ${estimate?.total != null ? money(estimate.total) : 'personal quote required'}. Materials allowance (tile, setting material, protection/tape, outlet risers and screws): ${estimate?.materials != null ? money(estimate.materials.total) : 'personal quote required'}. Planning total (labor + materials allowance): ${estimate?.projectTotal != null ? money(estimate.projectTotal) : 'personal quote required'}. Materials allowance is a planning budget, not a fixed price; actual costs may be higher or lower depending on the products selected. Labor changes only if the selected area, tile type, pattern, or project scope changes. Minimum project pricing covers preparation, setup, and finishing visits. Grout, caulk, trim, drywall supplies, tax, delivery, and disposal are not included in the materials allowance. Please confirm my quote.`;
